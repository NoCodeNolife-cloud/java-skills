...
public class Main {
	...
	public static void init() throws Exception {
		...
		doc = db.parse(new FileInputStream(new File("./src/main/resources/target.xml")));
	...
	public static void getAttrEles() throws XPathExpressionException {
		NodeList nodeList = (NodeList) xpath.evaluate("//bookstore/book[price>35.00]/title", doc, XPathConstants.NODESET);
		for (int i = 0; i < nodeList.getLength(); i++) {
			System.out.print(nodeList.item(i).getNodeName() + "-->" + nodeList.item(i).getTextContent() + " ");
		}
		System.out.println();
